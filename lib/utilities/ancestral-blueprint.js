'use strict';

const Blueprint = require('ember-cli/lib/models/blueprint');
const path = require('path');
const coffeeScriptPath = path.resolve(__dirname, '../..', 'blueprints');

/**
  Find the non-coffeescript version of the blueprint to bootstrap off
 */
module.exports = function (dasherizedName, project) {
  let projectPaths = project ? project.blueprintLookupPaths() : [];

  projectPaths = projectPaths.filter(function (p) {
    return p !== coffeeScriptPath;
  });

  projectPaths = projectPaths.concat(legacyBlueprintsPath());

  let blueprint = Blueprint.lookup(dasherizedName, {
    paths: projectPaths,
  });

  blueprint.project = project;

  return blueprint;
};

function legacyBlueprintsPath() {
  return path.join(
    path.dirname(require.resolve('ember-cli-legacy-blueprints')),
    'blueprints'
  );
}
