...
const CoffeeLint = require('@coffeelint/cli').lint;
...
function CoffeeScriptLinter(inputTree, options) {
  if (!(this instanceof CoffeeScriptLinter))
    return new CoffeeScriptLinter(inputTree, options);
  options = options || {};
  ...
  this.formatter = function (filePath, lintResults) {
    if (lintResults.length) {
      ...
      lintResults.forEach(function (error, index) {
        ...
        console.log(
          errorColor(' ' + error.message + ' (' + error.level + ') ')
        );
  ...
  this.statsFormatter = function (stats) {
    let color = chalk.black.bgGreen;
    if (stats.errorCount > 0) {
      color = chalk.white.bgRed;
    }
    console.log('\n\n=======================================================');
    console.log(
      'Linting completed on ' +
        stats.fileCount +
        ' files with ' +
        color(' ' + stats.errorCount + ' ') +
        ' errors.'
    );
    console.log('=======================================================\n');
  };
  ...
  let rawCoffeeLintConfiguration = null;
  ...
  if (rawCoffeeLintConfiguration) {
    try {
      ...
      throw (
        'Unable to parse json file: ' + path.resolve(options.coffeelintJSON)
      );
    }
  }
...
CoffeeScriptLinter.prototype.processString = function (content, relativePath) {
  if (this.coffeelintignore) {
    let shouldPass = this.coffeelintignore(relativePath);
    if (!shouldPass) {
      return content;
    }
  }
  try {
    let lintResults = CoffeeLint(content, this.coffeelintJSON);
...
CoffeeScriptLinter.prototype.cacheKeyProcessString = function (
  string,
  relativePath
) {
  return (
