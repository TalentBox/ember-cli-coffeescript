    ...
    return emberNew()
      .then(() =>
        emberGenerateDestroy(args, (_file) => {
          ...
          expect(_file('app/templates/foo.hbs')).to.contain('{{outlet}}');
          ...
          expect(_file('tests/unit/models/foo-test.coffee')).to.contain(
            "moduleForModel 'foo'"
          );

          expect(file('app/router.coffee')).to.contain("@route 'foo'");
        })
      )
      .then(() =>
        expect(file('app/router.coffee')).to.not.contain("@route 'foo'")
      );
  });
});
